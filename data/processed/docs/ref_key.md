...
```
To select particular files, use [`like`](../like/)
...
Where `x` is a vector returns the name of its [type](../../basics/datatypes/) as a symbol.
...
Where `x` is a non-negative integer returns the same result as [`til`](../til/).
