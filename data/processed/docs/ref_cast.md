...
```
  ...
  * an **upper-case letter** or a **negative short int** interprets the value from a string, see [Tok](../tok/)
...
`$`(cast) is a [multithreaded primitive](../../kb/mt-primitives/).

## Iteration¶

Cast is an [atomic function](../../basics/atomic/).
...
2015 28 10 3 55 58i
```
The following shows which information can be extracted from each temporal [datatype](../../basics/datatypes/).
...
Numeric calculations on the base numeric type can also be used to extract part of a temporal type, for the example using the [`mod`](../mod/) keyword:
...
1 1
```
For string values of `y`, see [Tok](../tok/).
...
[Overloads of `$`](../overloads/#dollar)   


_Q for Mortals_ [§7.2 Cast](/q4m3/7_Transforming_Data/#731-data-to-strings)
