...
dev x     dev[x]
```
Where `x` is a numeric list, returns its [standard deviation](https://en.wikipedia.org/wiki/Standard_deviation "Wikipedia") (the square root of the variance). Applies to all numeric data types.
...
b| 2
```
`dev` is a [multithreaded primitive](../../kb/mt-primitives/).
...
`mdev` applies to [dictionaries and tables](../../basics/math/#dictionaries-and-tables).
...
sdev x     sdev[x]
```
Where `x` is a numeric list, returns its sample standard deviation, the square root of the [sample variance](../var/#svar).
...
b| 2.828427
```
`sdev` is a [multithreaded primitive](../../kb/mt-primitives/).
...
[Sliding windows](../../kb/programming-idioms/#how-do-i-apply-a-function-to-a-sequence-sliding-window)   
