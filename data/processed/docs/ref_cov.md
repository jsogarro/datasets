...
x cov y    cov[x;y]
```
Where `x` and `y` are [conforming](../../basics/conformable/) numeric lists, returns their [covariance](https://en.wikipedia.org/wiki/Covariance "Wikipedia") as a floating-point number. Applies to all numeric data types.
...
`cov` is a [multithreaded primitive](../../kb/mt-primitives/).
...
`scov` is a [multithreaded primitive](../../kb/mt-primitives/).
