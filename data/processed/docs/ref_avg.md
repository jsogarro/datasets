...
The mean of an atom is its value as a float. Null is returned if `x` is empty, or contains both positive and negative infinity.
...
```
`avg` is a [multithreaded primitive](../../kb/mt-primitives/).
...
`wavg` is a [multithreaded primitive](../../kb/mt-primitives/).

## Implicit iteration¶

`avg`, `avgs`, and `mavg` apply to [dictionaries and tables](../../basics/math/#dictionaries-and-tables). `wavg` applies to dictionaries.
