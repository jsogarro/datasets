...
Publish-subscribe messaging with Solace | kdb+ and q documentation 
...
[ ![logo](../../local/img/kx.svg) ](../.. "kdb+ and q documentation") kdb+ and q documentation 
  ...
  * kdb+ and q  kdb+ and q 
    ...
    * Interfaces  Interfaces 
      ...
      * Solace pub/sub  [ Solace pub/sub ](./) On this page 
...
This paper covers different ways applications communicate with each other, the publish/subscribe (pub/sub) messaging pattern and its advantages, benefits of implementing pub/sub with kdb+, and finally, how you can implement pub/sub with kdb+ using Solaceâs PubSub+ event broker.
...
This is just one example of why applications need to talk to each other. There are many others but at the core of it, each system architecture consists of multiple types of applications â databases, caches, load balancers, API gateways etc, which they cannot exist in isolation. They need to talk to each other. Most applications need to store/retrieve data into/from a database. Most Web applications are a front end with a load balancer that routes Web traffic to various backend servers. These applications communicate with each other to make the overall architecture work. 

But how? 

### Interprocess communication¶

Applications directly communicating with each other is known as [interprocess communication](../../basics/ipc/) (IPC). A typical kdb+ architecture consists of several kdb+ processes sharing data with each other using IPC. A kdb+ server process listens on a specific port; a client kdb+ process establishes a connection with the server process by opening a handle:
...
With so many messaging patterns to choose from, why invest time and resources in implementing the pub/sub messaging pattern in your [architecture](../../architecture/)? Pub/sub has numerous advantages that make your architecture efficient, robust, reliable, scalable and cloud-ready.
...
Using an event broker puts the kdb+ estate at the center of your big data ecosystem and allows different teams to exploit its capabilities by integrating with it. Donât let your kdb+ estate sit in isolation! 
...
Solaceâs PubSub+ broker is heavily used in financial services. It supports open APIs and protocols, dynamic topics, wildcard filtering, and event mesh. Its support for high-throughout messaging and low latency makes it a suitable companion for kdb+. Some core features:
...
On Solace Cloud, we can create a free service very quickly by selecting the âfreeâ tier and picking AWS as our cloud provider. Below I have selected `US East` as my availability zone and named my service `demo`. Because this is a free tier, we are given a very lightweight PubSub+ instance with the following configuration:
...
With the API installed, you are ready to start using it from q. First connect to the Solace Cloud service created in the previous section. 
...
To establish a connection, we can use the `sol_capabilities.q` example. But first add our connection information to `sol_init.q`.
...
Letâs publish `"Hello, world"` to the topic `data/generic/hello`.
...
This specific topic hierarchy is used to exploit Solace PubSub+âs rich topic hierarchy, which provides strong wildcard support and advance filtering logic. 
...
The simulator uses the Solace JMS API to publish direct messages to PubSub+. (Solace also offers a proprietary Java API.)
...
As you can see, it is fairly simple to implement an RDB that consumes data from a PubSub+ broker rather than from from another kdb+ process. And multiple processes can consume the data without changing any existing processes. 
...
[`solace-pubsub.q`](assets/solace-pubsub.q "Download") Complete script
...
Besides being stored locally in the `stats` table, these minutely stats are also being published to the PubSub+ broker. We can see that by starting another process that subscribes to the `EQ/stats/>` topic.
...
Other papers by Himanshu Gupta
    ...
    * [How your application architecture has evolved](http://abitdeployed.com/2019/11/12/how-your-application-architecture-has-evolved/)
    * [Streaming market data simulator for your projects](http://abitdeployed.com/2020/03/29/streaming-market-data-simulator-for-your-projects/)
    * [q/kdb+ API for getting data from IEX Cloud](http://www.enlistq.com/q-kdb-api-for-getting-data-from-iex-cloud/)
    * [Enabling a modern hybrid cloud kdb+ stack with PubSub+](http://www.enlistq.com/enabling-a-modern-hybrid-cloud-kdb-stack-with-pubsub/)
