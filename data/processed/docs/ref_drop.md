# Drop – Reference – kdb+ and q documentation

## `_` Drop

"Drop items from a list, entries from a dictionary or columns from a table."

Syntax: `x _ y` or `_[x;y]`

Note: `_` (drop) is a multithreaded primitive.
...
q)(b?":") _ b / find the first ":" and remove the prior portion
": banana: cherry"
```
...
**Returns:** `x` without items or entries at `y`.
...
**Returns:** `y` without entries for `x`.
