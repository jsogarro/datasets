...
`hcount``hdel`The first one-and-done operation that works directly on a symbolic file handle is hcount, which returns a long representing the size of the file in bytes as reported by the OS.
...
`set`A serialized q data file can be read using (an overload of) the unary get, whose argument is a symbolic file handle and whose result is the q entity contained in the data file.
...
`set`Open a data file handle with hopen, whose result is a function called the open handle. This function should be stored in a variable, traditionally h, which is functionally applied to data to write it to the file. We will explain the result of applying the open handle shortly. We begin with a file containing serialized q data and show how to append to it.
...
`hclose`Failure to do so may cause your program to run out of file handles unnecessarily.
...
Surely, you say, we canât use an int as a function to write data. But you would be wrong.
...
`save``load`In its simplest form, save serializes a table in a global variable to a binary file having the same name as the variable. It overwrites an existing file.
...
`set`As you might expect, load is the inverse of save meaning that it reads a serialized table from a file into a variable with the same name as the file. It creates the variable in the workspace or overwrites it if it already exists.

`load``save`In a fresh q session after t has been saved as above,
...
`save`All the following versions of save can also be performed with the more general 0: â see Â§11.5.

`save``0:`Save the table with .txt extension to obtain tab-delimited records. There is no corresponding load but you can parse the text file â see Â§11.5.1.
...
Save the table with .csv extension to obtain comma-separated values. There is no corresponding load but you can parse the CSV file â see Â§11.5.2.
...
Save the table with .xml extension to obtain XML records. There is no direct way to read XML into q although libraries have been contributed â see code.kx.com.
...
It is worthwhile looking up the origin of the English word âsplayâ. Also, please donât spay your tables.
...
Nearly all the metadata regarding the splayed table can be read from the file system â i.e., the name of table from directory and names of the columns from the files. The one missing bit is the order of the columns, which is stored as a serialized list in the hidden .d file.
...
`set`The convention for enumerating symbols in splayed tables is to enumerate all symbol columns in all tables over the domain sym and store the resulting sym list in the root directory â i.e., one level above the directory holding the splayed table. You can do this manually but practically no one does.
...
Only unofficially documented, .Q.en prepares a qualified table for splaying by enumerating all its symbol columns. The first argument is the symbolic file handle of the root directory for the persistent residence of the enumeration domain sym (no choice in the name). The second argument is a table. See Â§14.5.2 for more detail on its behavior.

`.Q.en``sym`Update: .Q is now documented at code.kx.com. Ed.

`.Q`## 11.4 Text Data¶

We have seen that q views a record in a binary data file as a list of bytes. Similarly, a record in a text file is viewed as a list of char â i.e., a string. Thus reading a text file results in a list of strings and you pass a list of strings to write to a text file.
...
`hopen``hclose`Just as with a binary data file, a symbolic text file handle can be opened with hopen. The result is again an int that is conventionally stored in the variable h and is used with function application syntax to write data. The difference is that instead of using plain h to write binary data, you use neg[h] to write strings as text. Seriously.
...
`hclose``h``neg[h]`If the file already exists, opening with hopen and applying the open handle will append rather than overwrite.
...
We saw the built-in functions for saving tables as text files in Â§11.2. When you need to control the filename, you can write the table yourself with 0:, but then you must prepare the table columns as formatted text. A separate overload of 0: is available for this purpose. A confusing naming convention, to say the least.

`0:``0:`In this use, 0: has as left operand a char delimiter and as right operand a table or list of columns. Observe the use of the pre-defined constant csv, which is simply ",".
...
In the last snippet we applied 0: with two different meanings: to prepare and then write text. We hope youâve grown fond of this name, since Â§11.5 will introduce yet another version of 0: for parsing text records.

`0:``0:`## 11.5 Parsing Records¶
...
`0:``1:`The column labeled â0â contains the (upper case) field type char for text data. The (lower case) char in column â1â is for binary data. The column labeled âWidth(1)â contains the number of bytes that will be parsed for a binary read. The column labeled âFormat(0)â displays the format(s) that are accepted in a text read.
...
`(``;``) 1:`The left operand is a nested list containing two items: Lt is a simple list of char containing one letter per field; Lw is a simple list of int containing one integer width per field. The sum of the field widths in Lw should equal the width of the record. The result of the function is a list of lists, one list arising from each field.
...
`(``;``) 1:`The left operand is a list comprising two lists. Lt is a simple list of char containing one type letter per corresponding field. D is either a char representing the delimiting character or an enlisted char.
...
`0:`The following examples illustrate various combinations in Pf.
...
The ease with which a q process can communicate with another q process residing on the network is one of the most impressive features of q. We shall cover all the basics of interprocess communication (IPC) so that you can follow the section on callbacks in Chapter 1 â Q Shock and Awe.

We shall use the following terminology. The process that initiates the communication is called the client, while the process receiving and processing requests is the server. The server process can be on the same machine, the same network, a different network or on the Internet, so long as it is accessible. The communication can be synchronous (wait for a result to be returned) or asynchronous (donât wait and no result returned).
...
You can refer to a machine on the network by name. For example, on the authorâs laptop the following is equivalent to the two previous network handles.
...
`h`As you have no doubt realized, the application of h sent the string to the server to be evaluated. On the server, we see,
q)a
42
How cool is that?
...
We use the term map here to be any q expression that can be evaluated as function application â e.g., a list on an index, a dictionary on a key or a function on an argument. Most commonly f is a function

The remaining items arg1, arg2, â¦ are optional values sent along to the server for the evaluation. These are arguments when f is a function, indices when it is a list, or keys when it is a dictionary.

`f`Application of the connection handle to such a list sends the list to the server where it is evaluated. Any result is sent back to the client, where it is presented as the result of the connection handle application. By simply applying the naked handle, this sequence of steps is synchronous, meaning that execution of the q session on the client blocks until the result of the server evaluation is returned.
...
`f``f`On the client in our two-session setup:
...
`f`Allowing a function to be sent to the server for remote execution is as dangerous as sending quoted q strings
...
Now âcallâ the function f remotely from the client.
...
`::``hclose`If you do not do this, buffered messages may not be sent when the connection is closed.
...
`"Received 42"``"Returned 43"`When performing asynchronous messaging, always use neg[.z.w] to ensure that all messages are asynchronous

`neg[.z.w]`Otherwise you will get a deadlock as each process waits for the other.

You can override the default behavior of message processing in q by assigning your own handler(s) to the appropriate system variables. Assign your function to the variable .z.pg to trap and process synchronous messages and to .z.ps for asynchronous messages. The names end in âgâ and âsâ because synchronous processing has "get" semantics and asynchronous processing has "set" semantics.

`.z.pg``.z.ps`In the following we set the asynchronous handler to a trivial function, essentially ignoring asynchronous calls.
...
`"ignore"`Now we set the synchronous handler to a function that only accepts âsafeâ remote calls by function name. It then performs a protected evaluation on the function with the arguments passed, thus ensuring that a failed application does not hang the server.
...
In this section, we demonstrate how to execute q-sql queries against a remote server. First, we splay a table to stand for a time-series database. We use the mktrades script that we created in Â§9.3.1 to create a trades table with 1,000,000 rows and then splay it to disk.
...
We have already pointed out that allowing remote execution of arbitrary strings is bad practice because it exposes the server to injection attack. So here is a simplistic example of a âsafeâ function that can be used as a stored procedure. It takes a symbolic table name, a list of symbolic column names for the result and a date range for the where phrase. Enter on the server:
...
`.z.ph``.z.pp``.z.ph`There is no default handler for .z.pp.

`.z.pp`Here is a simple example that duplicates the default GET processing and shows the two items of its list argument. Define the following handler on the server process opened previously. It displays the two items of the input list then executes the first after removing the leading ? and then returns the result as a string.
...
And the browser page displays â42â.
...
`/pages/sample0.html`file:///pages/sample0.html
You should see a page with a single button labeled âget the answerâ. Click the button and you will see the answer in a very large font.
...
`sample1.html`For simplicity in the example, we have placed a copy of c.js in the pages directory. You should modify this to reflect its location in your installation.
...
`c.js`The script then defines JavaScript variables
...
`serverurl``c``connect``ws`The function connect() is where the WebSocket action happens.

`connect()`- It first tests to see if WebSocket is in the window, meaning that the browser supports WebSockets. If so, it makes the connection to the server; otherwise it displays an error alert.
...
`WebSocket``ws``binaryType``ws`Now we assign handlers for the main WebSockets events.
...
`payload``e``sayN`The sayN function locates the answer field on the page and places the text of its argument there. Finally, the script defines a simple HTML element answer.

`sayN``answer`In contrast, the server side q code is blissfully short. Start a fresh q session, open port 5042 and set the WebSockets handler .z.ws to a function that will be invoked to handle WebSockets messages.
...
The handler first deserializes its parameter and displays it to the console for debugging, at which point we have no further use for it in this example. Then it serializes the answer to the question asked by the browser and asynchronously sends it back to the browser. Thatâs all there is to it!
...
and you will see the answer displayed on the page. At this point you are equipped to follow Â§1.19 in Q Shock and Awe.
...
In WebSockets the browser initiates the connection, but once the WebSocket request for protocol upgrade is successful, the browser â i.e., client â and the server are on equal footing. Either side can send messages. Moreover, in the current q implementation of WebSockets all interaction is asynchronous. Given that most current browsers and the default q session are both single-threaded, you donât have to worry about races and deadlocks but you do have to set up callbacks.
...
`sample1.html``onmessage``sample2.html````q
<!doctype html>
<html>
<head>
<script src="c.js"></script>
...
Here is whatâs happening in the q code.
...
`answer``.z.w`Change since q3.2

âThe .z.wo and .z.wc message handlers were introduced in kdb+ version 3.3 (2014.11.26) to be evaluated whenever a WebSocket connection is opened (.z.wo) or closed (.z.wc). Prior to this version, .z.pc and .z.po provide an alternative solution however, these handle the opening and closing of all connections over a port and donât distinguish WebSocket connections.â 
â Whitepaper Kdb+ and WebSockets

`.z.wo``.z.wc``.z.wo``.z.wc``.z.pc``.z.po`See Reference: System and callbacks
