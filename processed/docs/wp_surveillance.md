  ...
  * kdb+ and q  kdb+ and q 
    ...
    * Learn  Learn 
      ...
      * Application examples  Application examples 
        ...
        * Trading surveillance  [ Trading surveillance ](./) On this page 
          ...
          * Bibliography 
          * Authors 
          * Appendix A â KX solutions 
          * Appendix B â market data 
...
by Sam Stanton-Cook, Ryan Sparks, Dan OâRiordan & Rob Hodgkinson
...
This metric is designed to detect a technique known as âfishingâ whereby HF traders rapidly create and cancel orders to test the market within the spread. To calculate the cancellation rate we count the number of orders that each broker created and cancelled under a specified holding time. The example below illustrates the number of orders that each broker held for less than 1 millisecond. While the technique is not necessarily manipulative, it can intentionally coerce other participants and their algorithms to enter the market. Seeing as these orders are unlikely to be filled by fundamental investors they add no benefit to the liquidity or spread. Additionally the high message turnover can be disruptive to the market and its participants. High cancellation rates within the spread can be indicative of quote-stuffing (described below in _Examining potentially harmful strategies_).
...
_Figure 2: Column chart displaying the number of orders per broker that were held for less than 1 millisecond visualized via Dashboards for KX. The selection only shows brokers that had at least 750 orders cancelled within the specified holding time. All codes have been scrambled._
    ...
    select count i by brokerID from dxOrder 
      where orderType in `new`cancelled, 
        00:00:00.001 > 1D^ ({x â prev x};transactTime) fby orderID
...
HF Traders can be further categorized by tracking the types of messages and the rate that a person of interest can send them. By calculating the time in between each message from a person of interest a profile can be drawn that details the messaging capabilities of a trader. Traders that demonstrate a left-skewed message rate histogram (majority of orders under 20ms) are likely to be high-frequency traders. Order distribution of a trader or participant assists in identifying frequent cancellations and âintent to tradeâ but perhaps with very few trades actually generated.

![Figure 5a](img/image7.jpeg) ![Figure 5b](img/image8.jpeg)  
...
Quote-stuffing is a technique whereby a trader floods the market with orders and rapidly cancels them. The ânoiseâ created by the influx of orders can affect other tradersâ latency and create an arbitrage opportunity for the high-frequency trader. If the orders are âstuffedâ within the spread, other traders can perceive a false impression of the market. Quote-stuffing generally occurs in bursts and will last less than 10 seconds on average (Credit Suisse, 2012).
...
Below we look at the skeleton function for finding price fades (not all lines of codes have been included, indicated by a ââ¦â):
...
Above we run the `priceFades` data on a list of dates using `peach` (with the process having started with `âs` set to enable secondary processes).
...
## Bibliography¶
  ...
  * Credit Suisse. (2012). _High Frequency Trading â Measurement, Detection and Response._
...
**Sam Stanton-Cook** , **Ryan Sparks** , **Dan OâRiordan** and **Rob Hodgkinson**

## Appendix A â KX solutions¶

A detailed list of the products referred to and demonstrated in this paper can be viewed at [Development tools](../../devtools/)
