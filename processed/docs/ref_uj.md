...
3 7| 30 C
```
`uj` is a [multithreaded primitive](../../kb/mt-primitives/).

`uj` generalizes the [`,` Join](../join/) operator.

Changes in V3.0

The union join of two keyed tables is equivalent to a [left join](../lj/) of the two tables with the catenation of unmatched rows from the second table.
...
_Q for Mortals_ [§9.9.7 Union Join](/q4m3/9_Queries_q-sql/#997-union-join)
