...
Columnar database and query optimization | documentation for q and kdb+ 
  ...
  * kdb+ and q  kdb+ and q 
    ...
    * Database  Database 
      ...
      * Maintenance  Maintenance 
        ...
        * Query optimization  [ Query optimization ](./) On this page 
...
Where appropriate, OS disk cache has been flushed using Simon Garlandâs `io.q` script. 
...
The purpose of this example is to examine the performance of queries with differently-ordered constraints as the database grows. In order to vary the size of the database, we will use [`.Q.view`](../../ref/dotq/#view-subview). To measure performance, we will use [`\ts`](../../basics/syscmds/#ts-time-and-space) to measure the execution time in ms and space used in bytes.
...
_Table 1: Results from queries as database grows_

![Figure 1](img/figure1.png)

_Figure 1: Effect of constraint order on execution times_
...
One result of interest is the performance for the non-optimal request in a database containing one date; we are only looking at one date, therefore one might expect that the performance would be similar. In order to explain the difference here we have to recall that date is a virtual column in the database â inferred by kdb+ by inspecting the folder names in the partitioned database. When the partition column is a constraint, but not the first constraint, kdb+ creates this data for comparison. What we are observing is the cost of this promotion and comparison.

![Figure 2](img/figure2.png)

_Figure 2: Effect of constraint order on workspace used_
...
_Table 2: Results from queries as more securities are requested_

![Figure 3](img/figure3.png)

_Figure 3: Effect of using precalculated data on execution times_
...
For certain aggregation functions, kdb+ has built-in map-reduce capabilities. When a query crosses partitions â for example a multi-day volume weighted average price (VWAP), kdb+ identifies whether the aggregation can be map-reduced. If so, it will perform the map operation across each of the partitions. Some aggregations that support map-reduce are `avg`, `count`, `max`, `min`, `sum`, `wavg`.
...
_Table 3: Results from queries using map-reduce_
...
_Figure 4: Map-reduce execution times as database grows_

It is possible to take advantage of this behavior with kdb+âs secondary processes, where the map calculation components are distributed to secondary processes in parallel, which can lead to very fast performance. The use of secondary processes is outside the scope of this paper.

## Use of attributes¶

[Attributes](../../ref/set-attribute/) can be applied to a vector, table, or dictionary to inform kdb+ that the data has certain properties that allow different treatments to be applied when dealing with that data.
...
_Table 4: Results from queries on data with parted attribute_
...
_Figure 5: Effect of parted attribute on execution times_
...
_Figure 6: Effect on workspace used of setting the parted attribute_
...
Letâs look at the impact of applying the sorted attribute on query performance as the dataset grows. We will be applying the attribute to the time column by sorting the data on time as we select it from the on-disk data. As we are not interested in the contents of the calculation results, we can combine data from multiple dates in the database and sort on time to simulate growing realtime tables. Between each request, we will restart kdb+.
    ...
    // â vary from 1 through 10 on each iteration
    rtquote:{
...
_Table 5: Results from queries on data with sorted attribute_

![Figure 7](img/figure7.png)

_Figure 7: Effect of sorted attribute on execution times_

We can see in the chart above the benefits from setting the attribute on the time column. While the request times without the attribute grow linearly with the increase in data, the times for the requests against the data with the attribute set are uniform â in this case returning instantly.

![Figure 8](img/figure8.png)

_Figure 8: Effect of sorted attribute on workspace used_

We observe similar results here for the amount of space used for the request. kdb+ is able to perform the calculation with a much smaller subset of the data. We can see that the curve representing space used for requests without an attribute set has a series of steps. These steps reflect the fact that kdb+âs buddy-memory algorithm allocates according to powers of 2.
...
_Table 6: Results from queries on data with unique attribute as table grows_

![Figure 9](img/figure9.png)

_Figure 9: Effect of unique attribute on execution times as table grows**_
...
For this example, weâll create a static last-value cache table and grow the number of sym values being requested.
...
_Table 7: Results as query grows from data with unique attribute set_

![Figure 10](img/figure10.png)

_Figure 10: Comparison of query times with unique attribute as query size grows_

NB: In order to observe the lower points on the curve, the âindex without attributeâ curve is not charted.
...
The grouped attribute is a conventional database âindexâ of all instances of a particular value. The ability of kdb+ to maintain this in real time as data is inserted allows fast searching, grouping and filtering on important fields.
...
_Table 8: Results as table grows from queries on data with grouped attribute set_

![Figure 11](img/figure11.png)

_Figure 11: Effect of grouped attribute on execution times as query grows**_
...
_Figure 12: Effect as table grows of grouped attribute on workspace used_
...
_Table 9: Results as query grows from queries on data with grouped attribute set_

![Figure 13](img/figure13.png)

_Figure 13: Comparison of query times with grouped attribute set as query size grows_

NB: omitted curve for âusing = and eachâ without attributes in order to examine more closely the faster-returning examples.
