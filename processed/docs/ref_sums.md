# sum, sums, msum, wsum – Sum Functions in kdb+
...
`sum` is equivalent to `+/`. It is a multithreaded primitive.

**Note on floating-point addition:** Different results may occur when changing summation order due to non-associativity:
...
`avg`, `min`, `max` and `sum` ignore nulls to align with SQL92 behavior. However, for nested `x`, these functions preserve nulls:
